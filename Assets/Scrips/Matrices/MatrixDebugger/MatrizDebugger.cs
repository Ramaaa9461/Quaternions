...
public class MatrizDebugger : EditorWindow
{
    public GameObject target;
    GUIStyle style = new GUIStyle();
    private Vector2 scroll;

    [MenuItem("Window/MatrizDebugger")]
    static void Init()
    {
        MatrizDebugger window = (MatrizDebugger)EditorWindow.GetWindow(typeof(MatrizDebugger));
        window.Show();
    }
    ...
    private void OnGUI()
    {
        ...
        if (target != null)
        {
            ...
            DrawMatrix(Matriz4x4.Translate(target.transform.position));

            EditorGUILayout.Space(40);

            EditorGUILayout.LabelField("Rotation matrix", style);
            EditorGUILayout.Space(10);
            DrawMatrix(Matriz4x4.Rotate(target.transform.rotation));

            EditorGUILayout.Space(40);

            EditorGUILayout.LabelField("Scale matrix", style);
            EditorGUILayout.Space(10);
            DrawMatrix(Matriz4x4.Scale(target.transform.localScale));

            EditorGUILayout.Space(40);

            EditorGUILayout.LabelField("TRS matrix", style);
            EditorGUILayout.Space(10);
            DrawMatrix(Matriz4x4.TRS(target.transform.position, target.transform.rotation, target.transform.localScale));
